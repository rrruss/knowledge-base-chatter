...
class PageSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        # Honoring 1 second delay per domain, and adhere to robots.txt.
        'DOWNLOAD_DELAY': 1,
        'ROBOTSTXT_OBEY': True,
        # Export to s3.
    ...
    def parse_metatags(self, response):
        description = ' '.join(response.xpath("//meta[@name='description']/@content").getall()).strip()
        keywords = ' '.join(response.xpath("//meta[@name='keywords']/@content").getall()).strip()
...
def scrape_sites(urls):
    ...
    print('Done processing {} urls in {:.2f} seconds'.format(len(urls), time.time() - start))

    # Load scrapy's data and remove local file
    result_df = pd.read_csv(LOCAL_OUTPUT_PATH, names=['url', 'description', 'body'])
    os.remove(LOCAL_OUTPUT_PATH)

    # If one of the stats do not exist, default 0.
    # This ensures assertions will not break due to missing keys.
    spider_stats = defaultdict(int)
    spider_stats.update(crawler.stats.get_stats())

    return result_df, spider_stats


def upload_to_s3(filename, result_df, spider_stats):
    now = datetime.now()
    timestamp = round(datetime.timestamp(now))
    date = now.strftime('%Y-%m-%d')

    result_df.to_csv(filename, header=False, index=False)


@inlineCallbacks
def main():
    # urls = read_urls_to_scrape(bucket, key)
    urls = ['https://slack.com/slack-tips/share-code-snippets',
            'https://slack.com/slack-tips/let-your-team-know-your-working-hours']
    result_df, spider_stats = yield scrape_sites(urls)
    upload_to_s3(LOCAL_OUTPUT_PATH, result_df, spider_stats)
    print(result_df.head(n = 25))
    reactor.stop()
...
if __name__ == '__main__':
    ...
    loop = LoopingCall(main)

    lcdeferred = loop.start(5) # every 5s call check_for_sqs callback if its not currently running
    lcdeferred.addErrback(lambda failure: loop_failed(failure, loop))

    sys.exit(reactor.run()) # be careful... twisted runs code asynchronously when Deferred objects are involved
